...
R2BO: Un agente reactivo, que es capaz de percibir el ambiente y responder
...
-Si tiene un niño en brazos y no hay mas niños sueltos se busca un camino hacia la
casilla con suciedad más cercana. 

-Si tiene un niño en brazos y se ha alcanzado el corral dejarlo.

-Si tiene un niño en brazos y hay mas sueltos ir a dejar el que tiene en el corral.

-Si no tiene ningún bebé consigo verificar si es mejor ir a perseguir uno o está más
carcano a una suciedad.

Note que este robot es bastante práctico, va tratando de recoer a los niños para que no
...
se anden moviendo mucho, pues así se alcanzrían antes.

Ideas seguidas para la implementación:

Para implementar las ideas descritas se utilizó el lenguajes de programación haskell como ya
se había comentado. 
...
cada uno con nu nombre y sus coordenadas. En el caso de los robots o los bebés se lleva también un
boleano wcompany que nos dice si están acompañados, o sea en el caso del rotbo si carga a alguien
...
se implementan los métodos generales de la simulación.
...
en valores random para decidir si efectuar un movimiento o no en un turno y la dirección del mismo
en cada de decidir realizarlo. Se asegura siempre que la dirección escogida está disponible y que
...
Para la generación de suciedad se analizan los niños que no estén cargados ni en un corral.
...
En función de este valor se genera tanta suciedad por dicho peque como sea posible, atendiedno al
...
Para el proyecto se utilizó stack, por lo que para efectuar una simulación se utiliza el comando
stack run. En el archivo Main.hs hay varios casos de prueba, se puede comentar y descomentar los que se
...
-Matriz resultante del movimiento aleatorio del medio, o sea con el movimiento de los beb'es y la suciedad
generada.
...
Consideraciones obtenidas a partir de la ejecución de las simulaciones del
problema:
...
Con simulaciones en tableros de 5X5 y un límite de tiempo de 11 segundos y con dos elementos de cada
tipo, los dos tipos de robots logran mantener el control de la situación, dejando siempre la casa
totalmente limpia o con un nivel de suciedad no superior al 4%. De igual forma, los bebés son casi siempre alcanzados en este tiempo. En tableros de 100 x 100 con 5 elementos de cada tipo se logra
...
Esto es entendible, puesto que se centra en capturar a los bebés y no limpia hasta que no haya alcanzado este objetivo. Notemos que aunque su actuar puede resultar eficiente cuando tiene bebés mas cerca y los captura antes de que se alejen, a la vez da más tiempo a que lo sbebés generen más suciedad
